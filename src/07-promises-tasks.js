...
function willYouMarryMe(isPositiveAnswer) {
  let promise;
  if (isPositiveAnswer) promise = Promise.resolve('Hooray!!! She said "Yes"!');
  if (isPositiveAnswer === false) promise = Promise.resolve('Oh no, she said "No".');
  if (!arguments.length) promise = Promise.reject(new Error('Wrong parameter is passed! Ask her again.'));
  return promise;
}
...
function chainPromises(array, action) {
  return new Promise((resolve) => {
    const resolvedArr = [];
    array.forEach((prom) => prom
      .then((data) => { resolvedArr.push(data); })
      .catch((err) => err));
    resolve(resolvedArr);
  })
